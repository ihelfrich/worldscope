...
| [`Farm-Bill-2026-Notes.pdf`](Farm-Bill-2026-Notes.pdf) | Single PDF: analysis (with charts) + all 12 title note-sets (~132 pp). |
| [`figures/`](figures/) | Six data-driven charts (provision-mix by title, money moves, loan limits, forestry acreage caps, bill anatomy). Regenerate with `tools/build_farmbill_figures.py`. |
...
python tools/build_farmbill_figures.py   # -> figures/*.png (charts from the notes)
python tools/build_farmbill_pdf.py       # -> Farm-Bill-2026-Notes.pdf (embeds the charts)
```

The first two charts are tallied **directly from the analysts' `Type:` tags** across all
460 sections, so they stay honest to the notes; the rest are transcribed from cited sections.
