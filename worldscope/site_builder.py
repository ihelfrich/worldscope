...
sections and the brief surfaces maybe 50 of them. Everything else is visible
only if a reader downloads the zip and grep-s the raw JSON.
...
/dist/assets/tailwind.css (built from assets/src/tailwind.input.css via the
npm `build:css` script). This module emits semantic markup; it does NOT ship
its own CSS.
...
# Pretty-print section IDs as titles. snake_case to Title Case + a few overrides
# for tighter naming.
PRETTY_NAMES = {
...
def safe_url(url: str) -> str:
    ...
    Defends against javascript:/data:/file: hrefs sneaking in through source
    data. Also rejects Google News RSS proxy URLs because they return raw XML
    to the browser rather than redirecting to the article.
    """
    if not url:
        return ""
    u = url.strip().lower()
...
def safe_path_segment(s: str) -> str:
    """URL-encode a path segment so quotes/spaces/reserved chars don't break
    links or inject attributes."""
...
def _navbar(base: str = "") -> str:
    """daisyUI-style navbar with mobile collapse driven by Alpine.js.

    `base` is the relative-URL prefix from the current page to dist/ (e.g.
    "../../" for /sections/<id>/<date>.html, "../" for /sections/<id>/, etc.).
    """
  ...
  <div class="mx-auto max-w-7xl px-5 lg:px-8 flex items-center gap-6 py-3">
    ...
    <a class="hidden lg:inline-block text-xs text-mist hover:text-white pl-4 ml-2 border-l border-white/20"
       ...
       aria-label="Personal hub (opens in new tab)">helfrich.github.io →</a>
...
def _read_jsonl(path: Path) -> list[dict]:
    """Load JSONL into a list of dicts. Non-dict lines and malformed JSON are
    silently skipped (counted in the caller if logging is wanted)."""
    if not path.exists():
        return []
    out: list[dict] = []
...
def _record_to_html(rec: dict) -> str:
    ...
    body_html = (
        f'<p class="ws-body">{html.escape(summary)}</p>' if summary else ""
    )
    ent_html = ""
    if entities_display:
        tags = "".join(
            f'<span class="ws-tag">{html.escape(e)}</span>'
            for e in entities_display[:8]
        )
        ent_html = f'<div class="ws-tags">{tags}</div>'
    return (
        f'<div class="ws-record">'
        f'<h3>{title_html}</h3>'
        f'<span class="ws-src">{html.escape(str(source_label) or "?")}</span>'
        + (f'<span class="ws-tier">{html.escape(str(tier))}</span>' if tier else "")
        + body_html
        + ent_html
        + "</div>"
    )
...
def _wrap(title: str, body: str, crumbs: list[tuple[str, str]],
    ...
    crumbs_html = '<span class="sep" aria-hidden="true">/</span>'.join(crumb_parts)
...
{_navbar(base=base)}
<div class="ws-shell">
  <nav class="ws-crumbs" aria-label="Breadcrumb">{crumbs_html}</nav>
  <main>
  {body}
  </main>
  <footer class="ws-foot mt-16 pt-5 border-t border-mist font-sans text-xs text-slate flex flex-wrap justify-between gap-3" role="contentinfo">
    <div>WORLDSCOPE · all records cited inline · raw bundles available per day</div>
    <div><a href="{base}index.html" class="text-navy hover:text-gold no-underline">↑ home</a></div>
  </footer>
</div>
...
def render_section_day(section_id: str, day: str, out_root: Path) -> Path:
    ...
    for label, recs in sorted(by_source.items(), key=lambda kv: -len(kv[1])):
        ...
        more = (f'<p class="font-sans text-xs text-slate mt-2">+{len(recs) - 60} more not shown on this page</p>'
                if len(recs) > 60 else "")
        anchor = re.sub(r"[^a-z0-9]+", "-", str(label).lower()).strip("-") or "src"
        sources_html_parts.append(
            f'<h2 id="src-{html.escape(anchor)}" class="mt-9 mb-4 pb-2 border-b border-mist">'
            f'{html.escape(str(label))} '
            f'<span class="font-sans text-sm text-slate font-normal ml-1">'
    ...
    body = (
        f'<h1>{html.escape(pretty_id)} '
        f'<span class="font-normal text-slate">· {html.escape(day)}</span></h1>'
        f'<p class="font-sans text-sm text-slate mt-2 mb-8">{html.escape(desc)} · '
        f'{n_total} record' + ("s" if n_total != 1 else "")
        + f' across {len(by_source)} source'
...
def render_section_index(section_id: str, out_root: Path,
    ...
    If `rendered_dates` is provided, only those dates get clickable links (the
    rest are shown as plain text with a "not yet built" marker). Avoids 404s
    from archive rows pointing at unrendered older dates.
    """
    ...
    for d in all_dates:
        ...
        if d in rendered:
            rows.append(
                f'<a class="ws-archive-row" href="./{html.escape(d, quote=True)}.html">'
                f'<span class="ws-date">{html.escape(d)}</span>'
                f'<span class="ws-n">{n_str}</span>'
                f'</a>'
            )
        else:
            rows.append(
                f'<div class="ws-archive-row opacity-60 cursor-default">'
                f'<span class="ws-date">{html.escape(d)}</span>'
                f'<span class="ws-n">{n_str} (archived only, no rendered page)</span>'
                f'</div>'
            )
    pretty_id = pretty_section(section_id)
    ...
    body = (
        ...
        f'<p class="font-sans text-sm text-slate mt-2 mb-8">{html.escape(desc)} · '
        f'{len(all_dates)} day' + ("s" if len(all_dates) != 1 else "")
        + " on file · "
...
def render_sections_root(out_root: Path) -> Path:
    ...
    for sid in section_ids:
        ...
        cards.append(
            f'<a class="ws-section-card" href="./{html.escape(sid_seg, quote=True)}/">'
            f'<div class="ws-name">{html.escape(pretty_section(sid))}</div>'
            f'<div class="ws-desc">{html.escape(desc)}</div>'
            f'<div class="ws-count">{len(dates)} day'
    ...
    body = (
        '<h1>Sections</h1>'
        '<p class="font-sans text-sm text-slate mt-2 mb-6">'
...
def render_404(out_root: Path) -> Path:
    ...
    body = (
        '<h1>404, not found</h1>'
        '<p class="font-sans text-sm text-slate mt-2 mb-6">'
        'That page is not in WORLDSCOPE. The brief is at '
        '<a href="/worldscope/" class="text-navy underline decoration-mist hover:decoration-gold">today\'s brief</a>; '
        'the section archives are at '
        '<a href="/worldscope/sections/" class="text-navy underline decoration-mist hover:decoration-gold">Sections</a>.'
        '</p>'
    )
    out_path = out_root / "404.html"
    out_path.write_text(
        _wrap("404", body, [("WORLDSCOPE", "/worldscope/"), ("404", "")],
              base="/worldscope/", canonical=f"{PAGES_BASE}/404.html",
