"""Send Pushover messages without confusing API acceptance with delivery.

Pushover can return HTTP 200 and ``status: 1`` while also reporting that the
account has no active devices.  That response is not a usable delivery.  This
module validates the response before it records a brief in the idempotency
file used by the GitHub workflow.
"""
...
class PushoverDeliveryError(RuntimeError):
    """The message did not reach an active Pushover destination."""
...
def _validated_request_id(response: Mapping[str, Any]) -> str:
    ...
    if info:
        # In particular: "no active devices to send to".  Refuse all
        # informational qualifications so a future soft-failure cannot be
        # mistaken for an unqualified accepted delivery.
        raise PushoverDeliveryError(str(info))
...
def deliver(
    *,
    ...
    """Send one message and return its accepted Pushover request id."""
    ...
    return _validated_request_id(response)
...
def send_and_record(
    *,
    ...
    """Record ``brief`` only after an active destination accepted the message."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.brief:
            request_id = send_and_record(
                brief=args.brief,
                sent_file=args.sent_file,
                **common,
            )
        else:
            request_id = deliver(**common)
