...
def publish_daily_ready(
    ...
    *,
    ...
    """Write a ready manifest only for a successful same-date bundle."""
    dist = Path(dist)
    ...
    if not isinstance(generated_at, str):
        raise ReadinessError("run report generated_at is missing")
...
def validate_daily_ready(
    ...
    *,
    ...
    """Validate the contract a downstream daily composer must enforce."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "publish-daily":
            ...
            manifest = _load_object(args.manifest, "readiness manifest")
            validate_daily_ready(
                manifest,
